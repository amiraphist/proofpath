...
function GraphBoard({ stageId, nodes, edges, selectedNodeId, onSelect, onRemove, onMove, onConnect, onConnectionUnavailable, onSound, alert, attackEvent }: { stageId: number; nodes: GraphNode[]; edges: { id: string; from: string; to: string; state: string }[]; selectedNodeId: string | null; onSelect: (id: string) => void; onRemove: (id: string) => void; onMove: (id: string, x: number, y: number) => void; onConnect: (from: string, to: string) => void; onConnectionUnavailable: (notice: string) => void; onSound: (name: "tap" | "connect-start" | "connect-done" | "error" | "delete") => void; alert?: boolean; attackEvent?: string }) {
...
export default function GameCanvas({ persistedCompleted = 0, onProgress }: { persistedCompleted?: number; onProgress?: (event: ProgressEvent) => void }) {
  ...
  const { mode, stage, stageIndex, session, stages, totalCompleted, selectStage, selectNode, moveNode, connectNodes, showConnectionNotice, addNode, removeNode, repair, undo, run, reset, nextStage } = useGameSession();
