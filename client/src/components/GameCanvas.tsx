...
function NodeCard({ node, selected, source, onSelect, onRemove, onStartDrag, onPort }: { node: GraphNode; selected: boolean; source: boolean; onSelect: () => void; onRemove: () => void; onStartDrag: (event: ReactPointerEvent<HTMLDivElement>) => void; onPort: (direction: "in" | "out") => void }) {
  ...
  return (
    <div data-node-id={node.id} className={`graph-node graph-node--${meta.color} ${selected ? "is-selected" : ""} ${source ? "is-source" : ""}`} style={{ left: `${node.x}%`, top: `${node.y}%` }} onClick={onSelect} onPointerDown={onStartDrag} role="button" tabIndex={0} aria-label={`${meta.label}. ${meta.help}`}>
      ...
      {node.type === "wallet" ? <span className="hardware-screen"><b>VERIFY</b><span>recipient · amount</span><i>SIMULATED SIGNER</i></span> : <span className="node-id">{node.id.replace("-", " · ")}</span>}
...
export default function GameCanvas() {
  ...
  return <main className="game-shell">
    ...
    <div className="game-layout">
      <aside className="mission-panel glass-panel"><div className="eyebrow"><span className="eyebrow-line" /> PAYMENT MISSION</div><div className="stage-select"><div><span className="muted-label">CURRENT STAGE</span><strong>{String(stage.id).padStart(2, "0")} <span>/ 10</span></strong></div><select value={stageIndex} onChange={(event) => selectStage(Number(event.target.value))} aria-label="Select stage">{stages.map((item, index) => <option key={item.id} value={index}>{String(item.id).padStart(2, "0")} — {item.title}</option>)}</select><ChevronDown size={15} /></div><div className={`severity severity--${stage.severity}`}><span /> {stage.severity.toUpperCase()} RISK</div><h1>{stage.title}</h1><p className="client-line">PAYMENT FILE <span>·</span> {stage.client}</p><p className="story">{stage.story}</p><p className={`brand-voice ${mode === "fix" ? "brand-voice--attack" : ""}`}>{mode === "fix" ? stage.fixFault : "Build the route before the payment can move."}</p><div className="objective"><span className="objective-icon"><ScanLine size={17} /></span><div><span className="muted-label">YOUR JOB</span><p>{stage.objective}</p></div></div><div className="lesson"><span className="muted-label">WHY IT MATTERS</span><p>{stage.lesson}</p></div><div className="progress-wrap"><div className="progress-label"><span>PAYMENT MISSIONS</span><strong>{totalCompleted} / 10</strong></div><div className="progress-bar"><span style={{ width: `${Math.max(4, (totalCompleted / 10) * 100)}%` }} /></div></div></aside>
